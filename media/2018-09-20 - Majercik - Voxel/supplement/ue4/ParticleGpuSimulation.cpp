...
//TODO restore
//Includes for file I/O
#include <fstream>
#include <iostream>
//INclude for ue4 printout
...
/** The texture size allocated for GPU simulation.
    These values control the number of particles that the GPU
    simulates. Adjust for number of voxels in model. Default value is 1024. -ZM*/
extern const int32 GParticleSimulationTextureSizeX = 4096;//2048;// 1024;
extern const int32 GParticleSimulationTextureSizeY = 8192;//2048;// 1024;
...
public:
    ...
    //TODO restore color
    FTexture2DRHIRef ColorTextureTargetRHI;
    FTexture2DRHIRef ColorTextureRHI;
	...
	virtual void InitRHI() override
	{
        ...
        //TODO restore voxel color
        // 16-bit per channel RGBA texture for velocity.
        check(!IsValidRef(ColorTextureTargetRHI));
        check(!IsValidRef(ColorTextureRHI));
        ...
        RHICreateTargetableShaderResource2D(
            ...
            /*bForceSeparateTargetAndShaderResource=*/ false,
            CreateInfo,
            ColorTextureTargetRHI,
            ColorTextureRHI
        );

        static FName ColorTextureName(TEXT("ParticleStateColor"));
        ColorTextureTargetRHI->SetName(ColorTextureName);
	...
	virtual void ReleaseRHI() override
	{
        ...
        //TODO restore voxel color
        ColorTextureTargetRHI.SafeRelease();
        ColorTextureRHI.SafeRelease();
	}
};
...
public:
	virtual void Bind( const FShaderParameterMap& ParameterMap ) override
	{
        ...
        //TODO restore color
        ColorTexture.Bind(ParameterMap, TEXT("ColorTexture"));
        ColorTextureSampler.Bind(ParameterMap, TEXT("ColorTextureSampler"));
...
public:
    ...
    //TODO restore voxel color
    FTexture2DRHIParamRef ColorTextureRHI;
...
void FGPUSpriteVertexFactoryShaderParametersVS::SetMesh(FRHICommandList& RHICmdList, FShader* Shader, const FVertexFactory* VertexFactory, const FSceneView& View, const FMeshBatchElement& BatchElement, uint32 DataFlags) const
{
    ...
    //TODO restore voxel color
    SetTextureParameter(RHICmdList, VertexShader, ColorTexture, ColorTextureSampler, SamplerStatePoint, GPUVF->ColorTextureRHI);
...
public:
		...
		: FGlobalShader(Initializer)
	{
        ...
        ColorTexture.Bind(Initializer.ParameterMap, TEXT("ColorTexture"));
        ColorTextureSampler.Bind(Initializer.ParameterMap, TEXT("ColorTextureSampler"));
		...
		)
	{
        ...
        //Typo was here
        SetTextureParameter(RHICmdList, PixelShaderRHI, ColorTexture, ColorTextureSampler, SamplerStatePoint, TextureResources.ColorTextureRHI);
...
public:
	...
	virtual void InitRHI() override
	{
		...
		// Stream 0.
		{
            ...
            // Color. //TODO restore voxel color
			...
			Elements.Add(FVertexElement(0, Offset, VET_Float4, 4, Stride, /*bUseInstanceIndex=*/ true));
			Offset += sizeof(FVector4);
			// ParticleIndex.
			Elements.Add(FVertexElement(0, Offset, VET_Float2, 5, Stride, /*bUseInstanceIndex=*/ true));
		...
		// Stream 1.
		{
			int32 Offset = 0;
			// TexCoord.
			Elements.Add(FVertexElement(1, Offset, VET_Float2, 6, sizeof(FVector2D), /*bUseInstanceIndex=*/ false));
			Offset += sizeof(FVector2D);
		}
...
public:
	 ...
	 * Set parameters for this shader.
	 */
	void SetParameters(FRHICommandList& RHICmdList, int32 InVisualizationMode, FTexture2DRHIParamRef PositionTextureRHI, FTexture2DRHIParamRef CurveTextureRHI, FTexture2DRHIParamRef VelocityTextureRHI)
	{
	...
	)
{
	...
	PixelShader->SetParameters(RHICmdList, VisualizationMode, StateTextures.PositionTextureRHI, CurveTextureRHI, StateTextures.VelocityTextureRHI);
	...
	for ( int32 Index = 0; Index < TileCount; ++Index )
	{
		...
		for ( int32 ParticleY = 0; ParticleY < GParticleSimulationTileSize; ++ParticleY )
		{
			for ( int32 ParticleX = 0; ParticleX < GParticleSimulationTileSize; ++ParticleX )
			{
                ...
                //TODO restore precision
                ParticleIndices->X = IndexX;//SetWithoutBoundsChecks(IndexX);
                ParticleIndices->Y = IndexY;//SetWithoutBoundsChecks(IndexY);					
...
public:
	...
	virtual void InitRHI() override
	{
		if ( RHISupportsGPUParticles() )
		{
            ...
            //TODO restore precision
			VertexBufferSRV = RHICreateShaderResourceView(VertexBufferRHI, BufferStride, PF_G32R32F);
		}
	}
};
...
class FGPUSpriteParticleEmitterInstance : public FParticleEmitterInstance
{
    ...
    /*Name of the model to load, "None" for non-model emitter*/
    FString ModelName;
...
public:
	...
	FGPUSpriteParticleEmitterInstance(FFXSystem* InFXSystem, FGPUSpriteEmitterInfo& InEmitterInfo)
		...
		, AllowedLoopCount(0)
        //TODO restore
        , ModelName(InEmitterInfo.RequiredModule->EmitterModel) //Initialize the emitter with the right voxel model
	{
	...
	virtual void Tick(float DeltaSeconds, bool bSuppressSpawning) override
	{
        ...
        UE_LOG(LogParticles, Warning, TEXT("Tick!"));
        ...
        UE_LOG(LogParticles, Warning, TEXT("PassedFirstReturn!"));
		// Grab the current LOD level
		UParticleLODLevel* LODLevel = GetCurrentLODLevelChecked();
		...
		if (bEnabled)
		{
            UE_LOG(LogParticles, Warning, TEXT("bEnabled"));
			...
			{
                UE_LOG(LogParticles, Warning, TEXT("SpawnParticles!"));
                ...
                static TArray<FVector> voxelPositions;
                static TArray<FVector> voxelColors;

                BurstInfo.Count = 0;
                if (ModelName != "None" && ((voxelPositions.Num() == 0) || !modelLoaded)) {

                    voxelPositions.Empty();
                    voxelColors.Empty();
                    //Search for the voxel file
                    TArray<FString> FilesInDirectory;
                    //We use FPaths::GameContentDir() instead of FPaths::ProjectContentDir() (online documentation is out of date).
                    //IFileManager::Get().FindFilesRecursive(FilesInDirectory, *FPaths::GameContentDir(), TEXT("huge-black-dragonSerialized"), true, false);

                    IFileManager::Get().FindFilesRecursive(FilesInDirectory, *FPaths::GameContentDir(), *ModelName, true, false);
                    //IFileManager::Get().FindFilesRecursive(FilesInDirectory, *FPaths::GameContentDir(), *FString("chr_bowSerialized"), true, false);

                    //Our FilesInDirectory should only have a single file.
                    //TODO restore
                    const FString voxFile = FilesInDirectory[0];

                    readPositionsAndColors(voxelPositions, voxelColors, voxFile);

                    modelLoaded = true;
                    //Debugging
                    //FFileHelper::SaveStringToFile(FString::FromInt(BurstInfo.Count), *(FString("c:/temp/debugVoxPositionSize.txt")));
                }
                if (ActiveParticles == 0 && ModelName != "None") {
                    BurstInfo.Count = voxelPositions.Num() - 1;
                }
                ...
                //Determine spawn count based in imported voxels todo restore
				...
				if (BurstInfo.Count > 0) //Currently called only once for voxel
				{

					// Spawn burst particles.
					BuildNewParticles(NewParticles.GetData() + FirstBurstParticleIndex, BurstInfo, ForceBurstSpawnedParticles, voxelPositions, voxelColors);

                }

                
				if (SpawnInfo.Count > 0) //Currently never called, for voxel
				{

					// Spawn normal particles.
					BuildNewParticles(NewParticles.GetData() + FirstSpawnParticleIndex, SpawnInfo, ForceSpawnedParticles, voxelPositions, voxelColors);
				}
...
private:
    ...
    static void readPositionsAndColors(TArray<FVector>& pos, TArray<FVector>& col, const FString& voxFile) {

        IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();

        IFileHandle* FileHandle = PlatformFile.OpenRead(*voxFile);

        if (FileHandle) {
            
            //FVectors for position and color;
            FVector position;
            FVector color;
            ...
            do {
                ...
                //FileHandle->Read requires uint8*
                ByteBuffer = reinterpret_cast<uint8*>(floatPtr);

            } while (FileHandle->Read(ByteBuffer, sizeof(int32)));

            delete FileHandle;

            //Debugging
            //FFileHelper::SaveStringToFile(FString::FromInt(readVoxels), *(FString("c:/temp/debugReadVoxels.txt")));
        }
	 ...
	 */
	void BuildNewParticles(FNewParticle* InNewParticles, FSpawnInfo SpawnInfo, TArray<FNewParticle> &ForceSpawned, TArray<FVector> positions, TArray<FVector> colors)
	{
		...
		for (int32 i = SpawnInfo.Count; i > 0; --i)
		{
            ...
            const float VoxelRadius = 25.0f;

            //Voxels are in G3D Y=up coordinate system, so translate to UE Z=up, then scale to voxel size
            //TODO restore
            NewParticle->Position = (positions.Num() > 0) ? FVector(positions[i][0], positions[i][2], positions[i][1]) * VoxelRadius * 2.0f : 
                TempParticle->Location + InterpFraction * EmitterDelta + SpawnInfo.StartTime * NewParticle->Velocity + EmitterInfo.OrbitOffsetBase + EmitterInfo.OrbitOffsetRange * RandomOrbit;
            //TODO restore
            NewParticle->Velocity = TempParticle->BaseVelocity;
            
           //Write -1 so we know whether or not to sample for color in the GPUSpriteFactoryShader.
            NewParticle->Color = (colors.Num() > 0) ? FVector4(colors[i], 1.0f) : FVector4(-1.0f,-1.0f,-1.0f,1.0f);
            /////VOXEL MODIFICATIONS/////
...
void FFXSystem::PrepareGPUSimulation(FRHICommandListImmediate& RHICmdList, FTexture2DRHIParamRef SceneDepthTexture)
{
	...
	FTextureRHIParamRef RenderTargets[3] =
	{
        ...
        CurrentStateTextures.ColorTextureTargetRHI

	};

	RHICmdList.TransitionResources(EResourceTransitionAccess::EWritable, RenderTargets, 3);
...
void FFXSystem::FinalizeGPUSimulation(FRHICommandListImmediate& RHICmdList, FTexture2DRHIParamRef SceneDepthTexture)
{
	// Grab resources.
    FParticleStateTextures& CurrentStateTextures = ParticleSimulationResources->GetCurrentStateTextures();
        //ParticleSimulationResources->GetVisualizeStateTextures();
	...
	RHICmdList.TransitionResources(EResourceTransitionAccess::EReadable, RenderTargets, 3);
	if (SceneDepthTexture)
	{
	...
	)
{
	...
	// Setup render states.
    FTextureRHIParamRef CurrentStateRenderTargets[3] = { CurrentStateTextures.PositionTextureTargetRHI, CurrentStateTextures.VelocityTextureTargetRHI, CurrentStateTextures.ColorTextureTargetRHI};
    FTextureRHIParamRef PreviousStateRenderTargets[3] = { PrevStateTextures.PositionTextureTargetRHI, PrevStateTextures.VelocityTextureTargetRHI, PrevStateTextures.ColorTextureTargetRHI};
	...
	{	
		...
		if ( !CurrentStateTextures.bTexturesCleared )
		{
            ...
            //TODO restore voxel color
            RHICmdList.BeginUpdateMultiFrameResource(CurrentStateRenderTargets[2]);
            ...
            //TODO restore voxel color
            SetRenderTarget(RHICmdList, CurrentStateTextures.ColorTextureTargetRHI, FTextureRHIRef(), ESimpleRenderTargetMode::EClearColorAndDepth);
            ...
            //TODO resotre color
            RHICmdList.EndUpdateMultiFrameResource(CurrentStateRenderTargets[2]);
		}
		...
		if ( !PrevStateTextures.bTexturesCleared )
		{
            ...
            //TODO restore color
            RHICmdList.BeginUpdateMultiFrameResource(PreviousStateRenderTargets[2]);
            ...
            //TODO restore color
            SetRenderTarget(RHICmdList, PrevStateTextures.ColorTextureTargetRHI, FTextureRHIRef(), ESimpleRenderTargetMode::EClearColorAndDepth);
            RHICmdList.CopyToResolveTarget(PrevStateTextures.ColorTextureTargetRHI, PrevStateTextures.ColorTextureTargetRHI, true, FResolveParams());
            ...
            //TODO restore color
            RHICmdList.EndUpdateMultiFrameResource(PreviousStateRenderTargets[2]);
		}
	}	
	...
	SetRenderTargets(RHICmdList, 3, CurrentStateRenderTargets, FTextureRHIParamRef(), 0, NULL);
	...
	if (NewParticles.Num())
	{
        ...
        //FString filename = "c:/temp/debugInject.txt";

        //FFileHelper::SaveStringToFile(FString(FString::FromInt(count) + " " + FString::FromInt(NewParticles.Num())), *filename);
        //count++;
        ...
        //TODO restore voxel color
        //Begin update of color resource
        //RHICmdList.BeginUpdateMultiFrameResource(InjectRenderTargets[2]);
	...
	RHICmdList.TransitionResources(EResourceTransitionAccess::EReadable, CurrentStateRenderTargets, 3);
	RHICmdList.EndUpdateMultiFrameResource(CurrentStateRenderTargets[0]);
    RHICmdList.EndUpdateMultiFrameResource(CurrentStateRenderTargets[1]);
	...
	if (SimulationCommands.Num() && FixDeltaSeconds > 0)
	{
        ...
        FTextureRHIParamRef VisualizeStateRHIs[3] = { VisualizeStateTextures.PositionTextureTargetRHI, VisualizeStateTextures.VelocityTextureTargetRHI, VisualizeStateTextures.ColorTextureTargetRHI};
		RHICmdList.TransitionResources(EResourceTransitionAccess::EWritable, VisualizeStateRHIs, 2);
		
		SetRenderTargets(RHICmdList, 3, VisualizeStateRHIs, FTextureRHIParamRef(), 0, NULL);
		RHICmdList.ApplyCachedRenderTargets(GraphicsPSOInit);
		ExecuteSimulationCommands(
					RHICmdList,
					GraphicsPSOInit,
		...
		RHICmdList.TransitionResources(EResourceTransitionAccess::EReadable, VisualizeStateRHIs, 3);		
	}
...
void FFXSystem::UpdateMultiGPUResources(FRHICommandListImmediate& RHICmdList)
{
	...
	{		
        ...
        //TODO restore voxel color
        //RHICmdList.CopyToResolveTarget(
        //    ParticleSimulationResources->GetCurrentStateTextures().ColorTextureTargetRHI,
        //    ParticleSimulationResources->GetCurrentStateTextures().ColorTextureRHI,
        //    /*bKeepOriginalSurface=*/ false,
        //    FResolveParams()
        //);
