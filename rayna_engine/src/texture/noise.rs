...
impl Texture for NoiseTexture {
    fn value(&self, scene: &Scene, intersection: &MeshIntersection, _rng: &mut dyn RngCore) -> Colour {
        /// Converts a float value output by the `noise` crate into a value we can use
        ...
        match self {
            ...
            Self {
                ...
                Colour::from([r, g, b].map(convert).map(|c| c as Channel))
            }
            Self {
                source: NoiseSource::WorldPos,
                colour: NoiseColour::Rgb { r, g, b },
            ...
            Self {
                ...
                let r = scene.get_noise3(r).value(intersection.pos_l.as_array());
                let g = scene.get_noise3(g).value(intersection.pos_l.as_array());
                let b = scene.get_noise3(b).value(intersection.pos_l.as_array());
                Colour::from([r, g, b].map(convert).map(|c| c as Channel))
            }
            Self {
                source: NoiseSource::UV,
